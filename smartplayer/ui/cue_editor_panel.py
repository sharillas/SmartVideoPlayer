...
class CueEditorPanel(QWidget):
    ...
    def _on_color_pick(self):
        ...
        if color.isValid():
            ...
            self._color_btn.setStyleSheet(f"background-color: {self._pending_color}; border: 1px solid #777; border-radius: 3px;")
            self._color_label.setText(f" {self._pending_color}")

    def _on_apply(self):
        self._apply_to_cue()
        self._save_btn.setText("APPLIED")
        self._save_btn.setStyleSheet(
